package com.pankti.webservicewithretrofit.utils.msprintsdk;



import static com.pankti.webservicewithretrofit.utils.msprintsdk.UtilsTools.convertToBlackWhite;
import static com.pankti.webservicewithretrofit.utils.msprintsdk.UtilsTools.encodeCN;
import static com.pankti.webservicewithretrofit.utils.msprintsdk.UtilsTools.encodeStr;
import static com.pankti.webservicewithretrofit.utils.msprintsdk.UtilsTools.isCN;
...
//    تعیین کنید که آیا چاپگر به درستی وصل شده است یا خیر
...
//     باز بودن هد چاپگر را مشخص کنید
...
//    دمای کاتر چاپگر و هد چاپ را تعیین کنید
...
//    تعیین اینکه آیا کاغذ چاپگر تمام شده است یا در شرف تمام شدن کاغذ است
...
//     خطای ظرف کاغذ 5 گیر کردن کاغذ 6 گیر کردن کاغذ 7 کاغذ کشیده شده 8 سنسور بیرون انداختن کاغذ دارای کاغذ است
    ...
    public static int CheckStatus(byte[] b_recv) {
        if ((b_recv[0] & 0x16) != 0x16)// 01 判断打印机是否连接正常  تعیین کنید که آیا چاپگر به درستی وصل شده است یا خیر
            return 2;
        if ((b_recv[1] & 0x04) == 0x04)// 02 判断打印机 机头打开  باز بودن هد چاپگر را مشخص کنید
            return 3;
        if ((b_recv[2] & 0x08) == 0x08)// 03 判断打印机 切刀 打印头温度  دمای کاتر چاپگر و هد چاپ را تعیین کنید
            return 4;
        if ((b_recv[2] & 0x40) == 0x40)
            return 5;
        if ((b_recv[2] & 0x20) == 0x20)
            return 6;
        if ((b_recv[3] & 0x60) == 0x60)// 04 判断打印机纸尽 纸将尽  تعیین اینکه آیا کاغذ چاپگر تمام شده است یا در شرف تمام شدن کاغذ است
            return 7;
    ...
    public static int CheckStatus1(byte bRecv) {
        if ((bRecv & 0x16) != 0x16)// 01 判断打印机是否连接正常  تعیین کنید که آیا چاپگر به درستی وصل شده است یا خیر
            return 2;
    ...
    public static int CheckStatus2(byte bRecv) {
        if ((bRecv & 0x04) == 0x04)// 02 判断打印机 机头打开 باز بودن هد چاپگر را مشخص کنید
            return 3;
        return 0;
    }

    public static int CheckStatus3(byte bRecv) {
        if ((bRecv & 0x08) == 0x08)// 03 判断打印机 切刀 打印头温度 دمای کاتر چاپگر و هد چاپ را تعیین کنید
            return 4;
        if ((bRecv & 0x40) == 0x40)
            return 5;
        if ((bRecv & 0x20) == 0x20)
    ...
    public static int CheckStatus4(byte bRecv) {
        if ((bRecv & 0x60) == 0x60)// 04 判断打印机纸尽 纸将尽 تعیین اینکه آیا کاغذ چاپگر تمام شده است یا در شرف تمام شدن کاغذ است
            return 7;
        if ((bRecv & 0x0C) == 0x0C)
            return 8;
        return 0;
    ...
    //10 04 05
    // 4 容纸器错误 5 堵纸  6 卡纸  7 拽纸 8 出纸传感器有纸// 4 خطای ظرف کاغذ 5 گیر کردن کاغذ 6 گیر کردن کاغذ 7 کاغذ کشیده شده 8 سنسور بیرون انداختن کاغذ دارای کاغذ است
    public static int CheckStatus5(byte bRecv) {
        if((bRecv & 0x80) == 0x80)
            return 4;	//容纸器错误、خطای کانتینر

        if((bRecv & 0x01) == 0x01)
            return 5;	//5 堵纸 //5 گیر کاغذ

        if((bRecv & 0x08) == 0x08)
            return 6;	//6 卡纸 مربا کاغذ

        if((bRecv & 0x02) == 0x02)
            return 7;	//7 拽纸 کاغذ بکشید

        if((bRecv & 0x04) == 0x04)
            return 8;	//8 出纸传感器有纸 در سنسور خروج کاغذ کاغذ وجود دارد
     ...
     * 返回值： 0 打印机正常、  1 打印机未连接或未上电、        2 打印机和调用库不匹配
     ...
     6 卡纸、            7 拽纸、           8 出纸传感器有纸
     3.43 وضعیت عملکرد ویژه چاپگر را دریافت کنید
     * [توضیحات: وضعیت عملکرد ویژه چاپگر را دریافت کنید، فقط برای برخی از مدل های D347 قابل استفاده است]
     * مقدار بازگشتی: 0 چاپگر عادی است، 1 چاپگر متصل نیست یا روشن نیست، 2 چاپگر و کتابخانه تماس گیرنده مطابقت ندارند
     3 چاپگر فعلی هیچ عملکرد خاصی ندارد، 4 خطای ظرف کاغذ، 5 گیر کردن کاغذ
     6 گیر کاغذ، 7 کاغذ کشیدن، 8 سنسور خروجی کاغذ
     * @return int
     */
...
//                strResult = "";
                break;
            case 1:
                strResult = "پرینتر خاموش است یا کاغذ ندارد";
                break;
            case 2:
                strResult = "Printer called unmatched library";
                break;
            case 3:
                strResult = "در چاپگر باز می باشد ";
                break;
            case 4:
                strResult = "کاتر ریست نمی شود";
                break;
            case 5:
                strResult = "دمای سر چاپگر غیر طبیعی است";
                break;
            case 6:
                strResult = "چاپگر علامت سیاه را تشخیص نمی دهد";
                break;
            case 7:
                strResult = " کاغذ تمام شده است  ";
                break;
            case 8:
                strResult = "کاغذ در حال اتمام است";
                break;
        }
        return strResult;
    }	
